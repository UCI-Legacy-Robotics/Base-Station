...
const styles = {
  HeaderContainer: "bg-[#0A2342] w-full flex flex-col flex-none p-2 pb-0 space-y-2 border-4 border-black",
  HeaderText: "text-white text-[1.3em] w-full h-full font-chivo font-bold flex items-center justify-center",
  MainContent: "w-full flex flex-col flex-grow overflow-hidden rounded-b-md pt-2",
  LabelBar: "h-12 w-full flex items-center justify-around flex-shrink-0 border-t-4 border-black",
  ScrollableContent: "flex-grow overflow-y-auto space-y-2",
  TextContent: "text-white w-full h-full font-chivo font-semibold flex items-center justify-center",
  LogEntry: "flex justify-between items-center bg-[#1D4156] rounded-md border border-black",
};
        ...
        <div className={styles.MainContent}>
          ...
          >
            ...
            {filteredEntries.map((entry, index) => (
              ...
              >
                <p className={styles.TextContent}>{entry.timestamp}</p>
                <p className={styles.TextContent}>{entry.sender}</p>
                <p className={styles.TextContent}>{entry.type}</p>
                <p className={styles.TextContent}>{entry.protocol}</p>
                <p className={styles.TextContent}>{entry.description}</p>
              </div>
            ))}
          </div>
        </div>
