...
const styles = {
  ...
  Selected: "bg-[#56656E]",
  AddRemove: "mt-2 bg-[#305E69] text-white rounded-md hover:opacity-60 w-[6rem] h-[2rem]",
  StopRemove: "mt-1 bg-[#9B111E] text-white font-russo-regular rounded-md hover:opacity-50 w-[7rem] h-[3rem]",
};
  ...
  const handleAdd = () => {
    if (longitude && latitude) {
      ...
      if (isNaN(parsedLongitude) || isNaN(parsedLatitude)) {
        alert("Please enter valid numeric values for longitude and latitude.");
        return;
      }

      setPinPoints([...pinPoints, { longitude: parsedLongitude, latitude: parsedLatitude }]);
      setLongitude("");
      setLatitude("");
    } else {
      ...
      <div className="h-full w-full grid grid-cols-1 md:grid-cols-[5fr_2fr] gap-4">
        ...
        */}
        <div className={styles.GNSSMap}>
          <h2 className="text-white font-chivo-semibold text-2xl mb-[0.8rem] text-[2.2em]">GNSS Map</h2>
